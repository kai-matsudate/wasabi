...
pub fn draw_test_pattern<T: Bitmap>(buf: &mut T) {
    ...
    for (i, c) in colors.iter().enumerate() {
        let y = i as i64 * h;
        fill_rect(buf, *c, left, y, h, h).expect("Failed to fill rect");
        fill_rect(buf, !*c, left + h, y, h, h).expect("Failed to fill rect");
    }

    let points = [(0, 0), (w, 0), (0, w), (w, w)];
    ...
    draw_str_fg(buf, left, h * colors.len() as i64 + 16, 0x00ff00, "ABCDEFG");
}
